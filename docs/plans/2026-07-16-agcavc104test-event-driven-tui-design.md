...
“全部点刷新”是一个逻辑批次：先对所有配置点各变化一次并生成不可变快照，再按 TypeID 分组、按 ASDU 容量拆帧并连续上送。不会在不同 TypeID 发送期间再次修改值。
...
控制表单至少包含命令类型、IOA、值或状态、选择位、公共地址。高级字段提供 OA、限定词、测试位、重复次数和发送间隔，但默认只发送一次。配置点提供类型和值域提示，不限制用户向未配置 IOA 发命令，以便观察 Java 的未知点、类型不匹配和非法值拒绝行为；重复发送可与静默应答组合，用于观察同链路 BUSY、队列背压及超时分支。
...
| 2. 采集日志 | 左侧接收、右侧发送；支持滚动、过滤和详细报文展开 |
| 3. 调度日志 | 调度链路收发及事务日志，保持与采集日志相同的视觉语义 |
| 4. 采集指令 | 只显示各类主动上送触发器，不显示执行结果 |
| 5. 调度指令 | 链路命令、召唤、校时、控制表单和兼容性探测 |
| 6. 采集实时值 | 按类型显示 IOA、值、品质、最后变化时间和配置名称 |
| 7. 调度实时值 | 分区显示最近一次完整总召和电度总召结果，突出 IOA 与值 |

页面通过注册表声明标题、快捷键和渲染函数，新增页签时不修改主事件循环。

采集指令结果全部进入采集日志；调度指令结果全部进入调度日志。Java 的调度主动变化上送也只进入调度日志，不混入第七页的总召快照。
...
数字量采用切换或明确的状态序列变化；模拟量在范围内按步长循环；电度按增量累加并维护序号。所有变化都通过领域点类型完成，禁止 UI 直接改数值。
...
第七页先列出配置中的预期 IOA，未收到时显示 `—`。Java 返回未配置 IOA 时动态追加并用浅黄色标记，但不丢弃数据。
